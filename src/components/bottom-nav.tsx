...
function Tab({
  ...
  return (
    <Link
      to={to}
      className={cn(
        "flex flex-col items-center justify-center gap-1 px-3 py-1 press-scale transition-colors duration-200",
        active ? "text-primary" : "text-muted-foreground",
      )}
    >
      <Icon
        className={cn("size-[22px] transition-transform duration-200", active && "scale-110")}
    ...
    <nav className="fixed bottom-0 inset-x-0 z-40 border-t border-border bg-card">
      <div className="mx-auto flex max-w-md items-center justify-between px-6 pt-2 pb-[max(env(safe-area-inset-bottom),0.75rem)]">
        ...
        <Link
          ...
          className="press-scale -mt-9 grid size-14 shrink-0 place-items-center rounded-2xl bg-primary text-primary-foreground border-4 border-background"
        >
